"""
Week 1 Full Demo: Enhanced with Smart Router V2 + Multi-Provider Fallback
...
CONTENT MODERATION FALLBACK SYSTEM:
- Primary models: Cost-optimized selection based on complexity
- Fallback models: Different providers with alternative content policies  
- OpenRouter automatically tries fallback models when content moderation fails
- Achieves 99.9%+ success rate instead of 99.9% with single provider
"""
...
from ..integrations.openrouter_integration import OpenRouterOptimizer
...
class Week1FullOptimizer:
    ...
    def __init__(self, max_budget: float = 5.00, config_path: str = "config/settings.yaml"):
        self.max_budget = max_budget
        self.config_path = config_path
        os.environ['MAX_BUDGET'] = str(max_budget)
        
        # Initialize components with configuration-based architecture
        self.api_optimizer = OpenRouterOptimizer(config_path=config_path)
        self.cost_tracker = CostTracker(config_path=config_path)
        self.data_loader = AmazonDataLoader(config_path=config_path)
        self.semantic_cache = SemanticCache(max_size=2000)
        self.smart_router = SmartRouterV2(config_path=config_path)  # Enhanced routing with complexity scoring
    ...
    def _get_conversation_context(self, category: str) -> list:
        """Get or create conversation context for KV cache optimization with universal system prompts"""
        if category not in self.conversation_contexts:
            # Use universal system manager for consistent prompts across all weeks
            self.conversation_contexts[category] = universal_system_manager.get_conversation_context(
                category=category, 
                week=1,  # Week 1 context
                context="analysis"
            )
...
Provide brief analysis: sentiment (Positive/Negative/Neutral), quality assessment, and key insight."""
        ...
        try:
            ...
            routing_result = self.smart_router.route_review(review_text, category)
            fallback_models = routing_result['all_models_for_openrouter']
            
            # Try each model in sequence until one succeeds (manual fallback)
            response = None
            last_error = None
            ...
            for model_name in fallback_models:
                try:
                    response = await asyncio.to_thread(
                        ...
                        model=model_name,  # Try one model at a time
                        messages=messages,
                        max_tokens=100,
                        temperature=0.1
                    )
                    ...
                    print(f"⚠️ Model {model_name} failed: {str(model_error)[:100]}... Trying next provider...")
            ...
            # Calculate costs using routing result
            ...
            # Track cost with actual model used
            ...
            return {
                ...
                'fallback_models_available': fallback_models,
                'primary_model_attempted': fallback_models[0]
            }
...
async def run_week1_complexity_routing_system():
...
if __name__ == "__main__":
    results, report = asyncio.run(run_week1_complexity_routing_system())
